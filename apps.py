...
import tempfile
import traceback
from lime.lime_text import LimeTextExplainer  # Added for explainability
...
def plot_to_base64(fig):
    img = io.BytesIO()
    fig.savefig(img, format='png', bbox_inches='tight', dpi=100)  # Reduced DPI for speed
    img.seek(0)
    plt.close(fig)
...
def generate_eda_plots(df, text_column, sentiment_column):
    ...
    try:
        ...
        if sentiment_column in df_sample.columns:
            ...
            if text:
                wc = WordCloud(width=250, height=125, background_color='white').generate(text)
                fig, ax = plt.subplots(figsize=(5, 2))
                ax.imshow(wc, interpolation='bilinear')
                ax.axis('off')
                ax.set_title(f"{most_frequent_sentiment} Word Cloud", fontsize=10)
                plots['word_cloud'] = plot_to_base64(fig)

        df_sample['text_length'] = df_sample[text_column].astype(str).apply(len)
        fig, ax = plt.subplots(figsize=(5, 3))
        sns.histplot(df_sample['text_length'], bins=15, kde=True, color='#4ecdc4')
        ax.set_title("Text Length Distribution", fontsize=10)
        plots['text_length'] = plot_to_base64(fig)
    except Exception as e:
        logging.error(f"Error in generate_eda_plots: {e}")
    return plots

def train_model(df, text_column, sentiment_column, split_ratio):
    try:
        ...
        y = df[sentiment_column].map(REVERSE_MAP)
        if y.isnull().all():
        ...
        return model, vectorizer, {
            ...
            'classes': list(SENTIMENT_MAP.values())
...
def upload_file():
    ...
    return render_template('data_preview.html', data=df.head().to_html(classes='table table-striped'),
...
def train():
    try:
        ...
        model_type = request.form.get('model_type', 'xgboost')  # Default to XGBoost
        ...
        model, vectorizer, metrics, counts_before, counts_after = train_model(df, text_column, sentiment_column, split_ratio)
        ...
        df_sample['predicted_sentiment'] = pd.Series(model.predict(X)).map(SENTIMENT_MAP)
        
        columns = df.columns.tolist()
        top_5 = df_sample[[text_column, 'predicted_sentiment']].head().to_dict('records')
        return render_template('data_preview.html', data=df.head().to_html(classes='table table-striped'),
...
def predict():
    try:
        ...
        if not text.strip():
            return render_template('data_preview.html', error="Please enter some text to predict.")
        
        if not os.path.exists('/tmp/model.pkl') or not os.path.exists('/tmp/vectorizer.pkl'):
            return render_template('data_preview.html', error="Model not trained yet. Please train a model first.")
        
        model = joblib.load('/tmp/model.pkl')
        vectorizer = joblib.load('/tmp/vectorizer.pkl')
        cleaned_text = clean_text_vectorized(pd.Series([text]))[0]
        X = vectorizer.transform([cleaned_text])
        pred = model.predict(X)[0]
        probs = model.predict_proba(X)[0]
        pred_label = SENTIMENT_MAP.get(pred, str(pred))  # Fallback to raw prediction if not in map
        ...
        return render_template('data_preview.html', data=df.head().to_html(classes='table table-striped'),
                               ...
                               result=f"Predicted: {pred_label}",
                               probs=dict(zip(SENTIMENT_MAP.values(), [f"{p:.2%}" for p in probs])),
                               input_text=text, summary=summary, columns=columns)
    except Exception as e:
...
def download():
    try:
        ...
        df_sample = df.sample(n=min(1000, len(df)), random_state=42) if len(df) > 1000 else df
        df_sample['cleaned_text'] = clean_text_vectorized(df_sample[text_column])
        df_sample['predicted_sentiment'] = pd.Series(model.predict(vectorizer.transform(df_sample['cleaned_text']))).map(SENTIMENT_MAP)
